...
export class ContentManagementComponent {
  ...
  openForm(crop: Crop | null = null) {
    ...
    if (crop) {
      ...
      crop.growthStages.forEach(stage => {
        const percentOfTotal = (stage.duration / crop.growthTime) * 100;
        cumulativePercent += percentOfTotal;

        // The last stage has 0 duration, handle it.
  ...
  async onSubmit() {
    ...
    // Transform form stages to game stages
    let lastPercent = 0;
