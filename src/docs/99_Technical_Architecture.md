...
-   **Firestore (قاعدة البيانات):**
    -   **بنية المجموعات (Collections Structure):**
        ...
        -   `admin_audit_logs`: لتسجيل الإجراءات الحساسة التي يقوم بها المسؤولون.
        -   `client_errors`: لتسجيل الأخطاء التي تحدث من جانب العميل تلقائيًا.
        -   `analytics_events`: لتسجيل أحداث سلوك اللاعبين الرئيسية.
...
## 6. المراقبة والتشخيص (Observability)

لضمان استقرار اللعبة وتوفير رؤى حول سلوك اللاعبين، تم بناء نظام للمراقبة.

-   **`GlobalErrorHandler`:** تم توفير معالج أخطاء مخصص على مستوى التطبيق. أي خطأ غير معالج (uncaught exception) يتم التقاطه بواسطة هذا المعالج وإرساله إلى `ObservabilityService` ليتم تسجيله.
-   **`ObservabilityService`:** خدمة مركزية مسؤولة عن:
    -   **تسجيل الأخطاء:** تجميع معلومات الخطأ (الرسالة، المكدس) مع سياق المستخدم (UID, email) وإرسالها إلى Firestore.
    -   **تسجيل الأحداث التحليلية:** توفير واجهة برمجية (`logEvent`) لتسجيل أحداث مهمة مثل `session_start` و `first_plant` لتتبع رحلة اللاعب.
-   **مجموعات Firestore:**
    -   `client_errors`: مجموعة مخصصة لتخزين سجلات الأخطاء.
    -   `analytics_events`: مجموعة مخصصة لتخزين الأحداث التحليلية.
-   **الهدف:** تمكين المطورين من مراقبة صحة التطبيق بشكل استباقي وتحليل بيانات اللعب لتحسين التوازن والتجربة.